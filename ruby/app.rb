...
=begin
        ret = Digest::SHA256.hexdigest "#{password}:#{salt}"
         db.xquery("UPDATE USERS SET calc_hash = ? WHERE password = ? salt = ?",
                 ret, password, salt)
=end
=begin
        row = db.xquery("SELECT calc_hash FROM USERS WHERE password = ? AND salt = ?  LIMIT 1;",password, salt ).first
        row['calc_hash']
...
=end
      ...
      def attempt_login(login, password)
        ...
        #if user && user['calc_hash'] == user['password_hash']
      ...
      def current_user
        ...
        @current_user = db.xquery('SELECT id FROM users WHERE id = ?', session[:user_id].to_i).first
        unless @current_user
          session[:user_id] = nil
          return nil
        end
      ...
      def last_login
        ...
        db.xquery('SELECT ip, created_at, login FROM login_log WHERE succeeded = 1 AND user_id = ? ORDER BY id DESC LIMIT 2', current_user['id']).each.last
      end
